...
class ARI:
    def __init__(self):
        ...
        self.store=Button(self.root,text='Save',
        					command=self.storage,
        					font=("Courier",18))
        self.searchb=Button(self.root,
        					textvariable=self.s,
        					command=self.mode,
        					font=("Courier",18))
        self.instructs=Label(self.root,
                        	text="Entry Mode",
                        	font=("Courier",18))
        self.tstr=StringVar()
        self.tstr.set("Title of your 'Text'")
        self.entry=Entry(self.root,
        					textvariable=self.tstr,
        					font=("Courier",18),
        					justify=CENTER)
        self.entry.bind("<FocusIn>",lambda x: self.entry.selection_range(0,END))
        self.text=Text(self.root,height=17)
        self.dstr=StringVar()
        self.details=Label(self.root,
        					textvariable=self.dstr,
        					justify=LEFT,
        					font=("Courier",12))
    ...
    def reading_age_of_text(self,text):
    	...
    	method=ceil(float((4.71*(characters/words))+(0.5*(words/sentences))-21.43))
    	if method<0:
    		method=0
    	return str(characters-1), str(words-1), str(sentences-1), str(method)
    ...
    def process_text(self):
    	...
    	self.dstr.set(des+c+' characters'+new+
    				des+w+' words'+new+
    				des+s+' sentences'+new+
    ...
    def search(self):
    	if self.tstr.get()=="Title of your 'Text'":
    		...
    		if self.tstr.get() in self.text.get("0.0",END):
    			...
    			self.text.tag_config('key',background='red',foreground='white')
    	if self.tstr.get()=='':
    ...
    def mode(self):
    	if self.s.get()=="Search Mode":
    		...
    		self.listoffiles.bind("<FocusIn>",lambda x: self.root.after_cancel(self.finish))
    		self.store.pack(side=RIGHT,fill=Y)
    		self.wide_search()
    		return
